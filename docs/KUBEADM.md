...
Then apply the ClusterIssuer. Note the solver field must be `ingressClassName`, not `class` — the `class` field is deprecated in cert-manager v1.x and silently does nothing.
